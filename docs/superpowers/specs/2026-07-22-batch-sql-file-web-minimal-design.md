...
`revision` 由 Web 进程内注册表按批次单调递增，每次权威快照变化都在同一注册表锁内先递增再广播。列表、详情、创建响应和 SSE 返回同一版本语义；客户端按 `batchId` 只接受 revision 不小于本地值的快照，以消除 GET、列表请求和 SSE 跨传输乱序造成的状态回退。
...
每次上传写入 `<upload-root>/<uuid>/<safe-original-basename>`：唯一目录保证同名文件互不覆盖，basename 保持用户可见文件名不带内部 UUID；批次继续只保存经过校验的服务端路径，不把 SQL 内容放进公开快照。
